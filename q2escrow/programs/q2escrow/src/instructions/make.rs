...
pub struct Make<'info> {
    ...
    // The maker's associated token account (ATA) for mint_a tokens
...
impl<'info> Make<'info>{
    ...
    pub fn transfer(&mut self, deposit:u64)-> Result<()>{
        //transfer the tokens from maker to vault
        ...
        let cpi_ctx = CpiContext::new(cpi_program, transer_accounts);
